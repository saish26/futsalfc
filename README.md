...
| `NEXT_PUBLIC_GOOGLE_CLIENT_ID` | Google **Web** OAuth client ID. Empty hides the Google button and shows a setup hint. |
...
3. Put the client ID in `NEXT_PUBLIC_GOOGLE_CLIENT_ID` here **and** in `google.client_id` in the API's `config.yml` — they must match.
